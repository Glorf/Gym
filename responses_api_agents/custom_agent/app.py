...
class CustomAgentConfig(SandboxCliAgentConfig):
    ...
    model_base_url_env: str = "OPENAI_BASE_URL"  # env var set to the in-box proxy URL
    api_key_env: str = "OPENAI_API_KEY"  # the in-box dummy key var
...
class CustomAgent(SandboxCliAgent):
    ...
    def build_launch(self, *, box_base_url, prompt, system_prompt, workdir, config_dir) -> LaunchPlan:
        env: dict[str, str] = {
            self.config.model_base_url_env: box_base_url,
            self.config.api_key_env: "dummy-key",  # pragma: allowlist secret — proxy injects the real key
            **self.config.box_env,
        }
