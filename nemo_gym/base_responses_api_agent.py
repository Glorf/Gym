...
class SimpleResponsesAPIAgent(BaseResponsesAPIAgent, AggregateMetricsMixin, SimpleServer):
    ...
    def setup_webserver(self) -> FastAPI:
        ...
        if self.agent_execution_coverage() is not None and self._agent_execution_store() is not None:
    ...
    def agent_execution_coverage(self) -> Optional[AgentExecutionCoverage]:
        """Return agent-side observability coverage, or None when this server emits no evidence."""
        return None
    ...
    def _agent_execution_store(self) -> Optional[AgentExecutionCaptureStore]:
        ...
        try:
            ...
            logger.warning("Invalid agent execution capture configuration; disabling it: %s", error)
            return None
        if not config.observability_enabled:
            return None
        assert config.model_call_capture_dir is not None
        try:
            store = AgentExecutionCaptureStore(config.model_call_capture_dir)
        except OSError:
    ...
    def _start_agent_execution_capture(self, body: Any) -> Optional[AgentExecutionRecorder]:
        coverage = self.agent_execution_coverage()
        if coverage is None:
            return None
        ...
        if self.agent_execution_capture_requires_single_worker() and (self.config.num_workers or 1) > 1:
            ...
            recorder.mark_incomplete("agent_execution_capture_requires_single_worker")
            return recorder
        recorder = AgentExecutionRecorder(rollout_id, self.config.name, coverage)
        lock, recorders = self._agent_execution_state()
        with lock:
            existing = recorders.get(rollout_id)
            if existing is not None:
                existing.mark_incomplete("duplicate_agent_execution_capture_owner")
                logger.warning(
                    "Agent execution capture already active for rollout %s; skipping duplicate owner.", rollout_id
                )
                return None
    ...
    async def _finish_agent_execution_capture(self, recorder: Optional[AgentExecutionRecorder]) -> None:
        ...
        if store is None:
            return
        try:
            await asyncio.to_thread(store.write, recorder.capture())
        except Exception:
    ...
    def _run_with_agent_execution_capture(self, run_handler):
        ...
        async def wrapped(*args, **kwargs):
            ...
            recorder = self._start_agent_execution_capture(body)
            try:
                return await run_handler(*args, **kwargs)
            finally:
    ...
    def adopt_agent_execution_capture(self, body: Any, capture: Mapping[str, Any]) -> None:
        recorder = self.agent_execution_recorder_for_run(body)
        if recorder is None:
            return
        try:
            recorder.adopt(AgentExecutionCapture.model_validate(capture))
        except Exception:
            recorder.mark_incomplete("invalid_worker_agent_execution_capture")
            logger.warning("Ignoring invalid worker agent execution capture.", exc_info=True)
