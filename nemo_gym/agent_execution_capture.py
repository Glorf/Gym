...
CaptureConfidence = Literal["exact", "partial", "unavailable"]
ToolSpanGranularity = Literal["individual", "batch"]
ToolMeasurementScope = Literal["caller_round_trip", "stream_observation_interval", "opaque_environment_step"]
ToolSpanStatus = Literal["returned", "raised"]


class AgentExecutionCoverage(BaseModel):
    """Static agent-server capability, not a claim that every event in a rollout was captured."""

    lineage: CaptureConfidence
    model_call_attribution: CaptureConfidence
    tool_timing: CaptureConfidence


class AgentInvocationRecord(BaseModel):
    """One agent invocation in the rollout's invocation tree."""
...
class ToolSpanRecord(BaseModel):
    """One observed individual tool call or one opaque tool batch."""
    ...
    granularity: ToolSpanGranularity
    ...
    def validate_span(self) -> "ToolSpanRecord":
        if self.ended_ns < self.started_ns:
            raise ValueError("ended_ns must not precede started_ns")
        if not self.tool_call_ids:
            raise ValueError("tool_call_ids must not be empty")
        if self.granularity == "individual" and len(self.tool_call_ids) != 1:
            raise ValueError("an individual tool span must reference exactly one tool call")
        return self


class ModelCallLink(BaseModel):
    """Attribution from an agent invocation to a captured model call."""
...
class AgentExecutionCapture(BaseModel):
    ...
    rollout_id: str
    agent_server: str
    coverage: AgentExecutionCoverage
    ...
    def validate_references(self) -> "AgentExecutionCapture":
        ...
        if sum(invocation.parent_id is None for invocation in self.agent_invocations) != 1:
            raise ValueError("agent invocations must contain exactly one root")
        ...
        for invocation in self.agent_invocations:
            ...
            while parent_id is not None and parent_id in invocation_by_id:
...
class AgentExecutionRecorder:
    """Thread-safe in-memory recorder flushed once by the owning Agent Server."""
    ...
    def __init__(
        ...
        *,
        ...
        self._agent_invocations = [AgentInvocationRecord(id=self.ROOT_INVOCATION_ID, source=agent_server)]
    ...
    def record_tool_span(
        ...
        *,
        ...
        with self._lock:
            span_id = f"tool-{self._next_span_index}"
    ...
    def tool_span(
        ...
        *,
        ...
        try:
            ...
            self.mark_incomplete("tool_span_recording_failed")
    ...
    def mark_incomplete(self, warning: str) -> None:
        """Record a known evidence gap without changing agent execution."""
        with self._lock:
            self._capture_complete = False
            if warning not in self._warnings:
                self._warnings.append(warning)

    def adopt(self, capture: AgentExecutionCapture) -> None:
        """Adopt evidence recorded by the sole worker for this rollout."""
        if capture.rollout_id != self.rollout_id:
            raise ValueError("worker capture rollout_id does not match its owning rollout")
        if capture.agent_server != self.agent_server:
            raise ValueError("worker capture agent_server does not match its owning rollout")
        with self._lock:
            self.coverage = capture.coverage
            self._capture_complete = capture.capture_complete
            self._agent_invocations = list(capture.agent_invocations)
            self._tool_spans = list(capture.tool_spans)
            self._model_call_links = list(capture.model_call_links)
            self._warnings = list(capture.warnings)
            self._next_span_index = len(self._tool_spans)
...
def merge_agent_execution_capture_into_record(record: dict, capture_dirs: list[Path]) -> dict:
    """Attach agent execution evidence without changing the agent response or reward."""
    rollout_id = maybe_rollout_id_from_run_body(record)
    ...
    for directory in capture_dirs:
        ...
        if capture is None:
            continue
        if capture.rollout_id != rollout_id:
            logger.warning(
                "Ignoring agent execution capture for rollout %s containing rollout id %s.",
                rollout_id,
                capture.rollout_id,
            )
            continue
