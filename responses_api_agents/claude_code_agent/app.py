...
class _ClaudeStreamObserver:
    ...
    def __init__(
        ...
        *,
        ...
        self.clock = clock
        self.pending_tools: dict[str, tuple[int, str, Optional[str]]] = {}
        self.pending_agents: dict[str, tuple[str, str]] = {}
        self.invocation_ids = {recorder.ROOT_INVOCATION_ID}
        self.linked_model_calls: set[tuple[str, str]] = set()
    ...
    def observe(self, event: dict[str, Any]) -> None:
        ...
        if invocation_id not in self.invocation_ids:
            pending_agent = self.pending_agents.pop(invocation_id, None)
            if pending_agent is None:
                self.recorder.mark_incomplete("claude_unknown_parent_tool_use_id")
                return
            source, parent_id = pending_agent
            self.recorder.add_agent_invocation(invocation_id, source=source, parent_id=parent_id)
        ...
        if event.get("type") == "assistant":
            ...
            if response_id is None and self.model_server is not None:
                self.recorder.mark_incomplete("claude_model_response_missing_id")
            if (
                self.model_server is not None
                and response_id is not None
                and (invocation_id, response_id) not in self.linked_model_calls
            ):
                ...
                self.linked_model_calls.add((invocation_id, response_id))
            ...
            for block in content:
                ...
                if not isinstance(tool_call_id, str):
                    self.recorder.mark_incomplete("claude_tool_use_missing_id")
                    continue
                if tool_call_id in self.pending_tools:
                    self.recorder.mark_incomplete("claude_duplicate_tool_use_id")
                    continue
                self.pending_tools[tool_call_id] = (self.clock(), invocation_id, response_id)

                if block.get("name") in {"Agent", "Task"}:
                    tool_input = block.get("input")
                    source = tool_input.get("subagent_type") if isinstance(tool_input, dict) else None
                    if tool_call_id in self.pending_agents or tool_call_id in self.invocation_ids:
                        self.recorder.mark_incomplete("claude_duplicate_agent_invocation")
                    else:
                        self.pending_agents[tool_call_id] = (str(source or block["name"]), invocation_id)
            ...
            for block in content:
                ...
                if not isinstance(tool_call_id, str):
                    self.recorder.mark_incomplete("claude_tool_result_missing_id")
                    continue
                pending = self.pending_tools.pop(tool_call_id, None)
                if pending is None:
                    self.recorder.mark_incomplete("claude_tool_result_without_start")
                    continue
                started_ns, owner_id, response_id = pending
    ...
    def finish(self) -> None:
        if self.pending_tools:
            self.recorder.mark_incomplete("claude_tool_result_missing")
...
class ClaudeCodeAgent(SimpleResponsesAPIAgent):
    ...
    def agent_execution_coverage(self) -> AgentExecutionCoverage:
        return AgentExecutionCoverage(
            lineage="partial",
            model_call_attribution="partial" if self.config.model_server else "unavailable",
            tool_timing="partial",
        )
        ...
        try:
            ...
            if execution_recorder is None:
                ...
                observer = _ClaudeStreamObserver(
                    execution_recorder,
                    model_server=self.config.model_server.name if self.config.model_server else None,
                )
                ...
                async def read_stdout() -> bytes:
                    ...
                    while line := await proc.stdout.readline():
                        ...
                        try:
                            ...
                            execution_recorder.mark_incomplete("claude_stream_event_invalid")
                        except Exception:
                            execution_recorder.mark_incomplete("claude_stream_observation_failed")
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                    execution_recorder.mark_incomplete("claude_code_timeout")
                    try:
                        await asyncio.wait_for(proc.wait(), timeout=5)
                    except asyncio.TimeoutError:
                        execution_recorder.mark_incomplete("claude_code_kill_timeout")
                try:
                    ...
                    execution_recorder.mark_incomplete("claude_stream_drain_timeout")
                    stdout_task.cancel()
                    stderr_task.cancel()
            ...
            if proc.returncode not in (0, None):
                if execution_recorder is not None:
                    execution_recorder.mark_incomplete("claude_code_nonzero_exit")
